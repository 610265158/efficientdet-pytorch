...
class DsfdDataIter():
    ...
    def crazy_crop(self, dp):
        ...
        yc, xc = [int(random.uniform(-x, 2 * s + x)) for x in
                  [-cfg.DATA.hin // 2, -cfg.DATA.win // 2]]  # mosaic center x, y
        labels4 = []
        for i, item in enumerate(holder):
            ...
            # place img in img4
            if i == 0:  # top left
                img4 = np.full((s * 2, s * 2, img.shape[2]), cfg.DATA.IMAGENET_DEFAULT_MEAN,
    ...
    def crazy_mix(self, image, boxes, labels):
        ...
        for k in range(boxes_2.shape[0]):
            ...
            box_produced = np.array([start_x, start_y, start_x + cur_width, start_y + cur_height, labels[k]])
